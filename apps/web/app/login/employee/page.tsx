...
export default function LoginPage() {
  ...
  // Custom handler for Step 1
  const handleCheckDomain = async () => {
    // 1. Safely grab the latest value directly from the form state
    ...
    try {
      ...
      if (response.data.sso_enabled) {
        ...
        const authUrl = `${keycloakUrl}/realms/${realm}/protocol/openid-connect/auth?client_id=${clientId}&redirect_uri=${redirectUri}&response_type=code&scope=openid&kc_idp_hint=${idpAlias}`;
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col items-center justify-center p-4">
      ...
      <Card className="py-2 w-full max-w-lg border border-border bg-card shadow-xl overflow-hidden transition-all duration-300">
        <CardHeader className="space-y-3 pb-8 pt-5 px-8 border-b border-border bg-muted/30 relative">
          ...
          {/* Use form.Subscribe here so the subtitle updates reactively too! */}
          <form.Subscribe
          ...
          >
            ...
            {step === 1 && (
              <div className="animate-in fade-in slide-in-from-left-4 space-y-6">
                ...
                {/* 2. Subscribe to the email field so the disabled state reacts instantly */}
                <form.Subscribe
                  selector={(state) => state.values.email}
                  children={(email) => (
                    <Button 
            ...
            {step === 2 && (
              <div className="animate-in fade-in slide-in-from-right-4 space-y-6">
                ...
                {/* Subscribe to the password field here as well */}
                <form.Subscribe
